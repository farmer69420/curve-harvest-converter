...
contract CurveConverterBUSD is ICurveConverter {
  ...
  * 3: BUSD
   */
  enum TokenIndex { DAI, USDC, USDT, BUSD }
  ...
  */
  function depositCurve(
    ...
    prepareTransfer(IERC20(busd), _amount[uint(TokenIndex.BUSD)]);

    ICurveBUSDDeposit(curve).add_liquidity(_amount, _minimum);
